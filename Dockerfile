...
CMD ["sh", "-c", "mkdir -p /app/data; for f in prompt_templates.json last_params.json generation_durations.json saved_videos.json; do [ -f \"/app/data/$f\" ] || cp \"/app/seed-data/$f\" \"/app/data/$f\" 2>/dev/null || true; done; exec uvicorn app.main:app --host 0.0.0.0 --port 8000"]
